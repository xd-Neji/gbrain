 ...
 *
 * Drift contract: every check name that ships through doctor.ts MUST appear
 * in exactly one set below. The drift-guard test in
 * `test/doctor-categories.test.ts` enforces this by reading doctor.ts plus
 * imported onboard check source via a tagged-string scan and asserting set
 * membership exactly.
 *
